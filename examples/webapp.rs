...
fn run() {
    use eframe::egui;
    use egui_hexweb::{random_symmetric_board, GameStatus, HexwebGame, HexwebWidget, Params};
    ...
    impl Preset {
        ...
        fn label(self) -> &'static str {
            match self {
                Self::Beginner => "Beginner (8 nodes)",
                Self::Intermediate => "Intermediate (12 nodes)",
                Self::Expert => "Expert (16 nodes)",
            }
        }

        /// `(nodes, pieces, arrow range)`. Difficulty comes from all three:
    ...
    wasm_bindgen_futures::spawn_local(async move {
        eframe::WebRunner::new()
            .start(
                ...
                Box::new(|cc| Ok(Box::new(HexwebApp::new(cc)))),
            )
            .await
